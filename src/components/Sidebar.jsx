...
import { isFeatureEnabled, getFeatureDescription } from '../config/rbac';
...
const NAV_ITEMS = [
  {
    ...
    path: '/',
    roles: ['admin'],
    description: 'Vista general del taller: solicitudes activas, métricas y estado del día.',
  ...
  },
  {
    ...
    description: 'Listado de todas las solicitudes de servicio: activas, en proceso y completadas.',
  ...
  },
  {
    ...
    roles: ['admin', 'mecanico'],
    description: 'Registra una nueva solicitud de servicio para un vehículo.',
  ...
  },
  {
    ...
    path: '/servicios',
    roles: ['admin'],
    description: 'Administra el catálogo de servicios y precios del taller.',
  ...
  },
  {
    ...
    path: '/seguimiento',
    roles: ['admin'],
    description: 'Analítica de repuestos: ranking de ventas, historial y stock por cliente.',
  ...
  },
  {
    ...
    path: '/catalogos',
    roles: ['admin'],
    description: 'Gestiona clientes, vehículos y mecánicos del taller.',
  ...
  },
  {
    ...
    path: '/reportes',
    roles: ['admin'],
    description: 'Genera reportes de ingresos, servicios y rendimiento del taller.',
...
export default function Sidebar({ open, onToggle, isOverlay }) {
  ...
  const [tooltip, setTooltip] = useState(null); // { name, description, locked, y }
    ...
    <aside className={sidebarClass}>
      ...
      <nav className="flex-1 py-4 px-2 space-y-1 overflow-y-auto overflow-x-visible">
        {navItems.map((item) => {
          ...
          const desc = item.description || getFeatureDescription(item.path) || item.name;
          const handleMouseEnter = (e) => {
            if (enabled) return;
            const rect = e.currentTarget.getBoundingClientRect();
            setTooltip({ name: item.name, description: desc, locked: true, y: rect.top + rect.height / 2 });
          };
      ...
      {tooltip && (
        <div
          className="pointer-events-none fixed left-[4.5rem] z-[9999] transition-opacity duration-150"
          style={{ top: tooltip.y, transform: 'translateY(-50%)' }}
        >
          <div className="bg-slate-900 border border-slate-600 text-slate-200 text-xs rounded-lg px-3 py-2 shadow-2xl w-56">
            <p className={`font-semibold mb-0.5 ${tooltip.locked ? 'text-amber-400' : 'text-white'}`}>
              {tooltip.name}{tooltip.locked ? ' · PRO' : ''}
            </p>
            <p className="text-slate-400 leading-snug">{tooltip.description}</p>
          </div>
        </div>
      )}
    </aside>
