  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <Link
          to="/nueva-solicitud"
          className="inline-flex items-center gap-2 bg-accent hover:bg-red-700 text-white font-semibold px-5 py-2.5 rounded-xl shadow-md hover:shadow-lg transition-all text-sm w-fit"
        >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-6 gap-3">
        <KpiCard label="Total"       value={kpis.total}       sub="todas las órdenes"    bg="bg-slate-100"   color="text-slate-600"   border="border-slate-100" icon="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
        <KpiCard label="Hoy"         value={kpis.hoyTotal}    sub="ingresaron hoy"       bg="bg-blue-50"     color="text-blue-600"    border="border-blue-100"  icon="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
        <KpiCard label="Pendientes"  value={kpis.pendientes}  sub="por atender"          bg="bg-amber-50"    color="text-amber-600"   border="border-amber-100" icon="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
        <KpiCard label="En proceso"  value={kpis.enProceso}   sub="en taller ahora"      bg="bg-orange-50"   color="text-orange-600"  border="border-orange-100" icon="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
        <KpiCard label="Completadas" value={kpis.completadas} sub="finalizadas"          bg="bg-green-50"    color="text-green-600"   border="border-green-100" icon="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
        <KpiCard label="Sin asignar" value={kpis.sinAsignar}  sub="sin mecánico"         bg="bg-red-50"      color="text-red-500"     border="border-red-100"   icon="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
      </div>

      {/* ── Layout 2 columnas ── */}
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-5">

        {/* ── Tabla solicitudes recientes (2/3) ── */}
        <div className="xl:col-span-2 bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
            <div className="flex items-center gap-2">
              <div className="w-7 h-7 bg-blue-50 rounded-lg flex items-center justify-center">
                <Icon path="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" className="w-3.5 h-3.5 text-blue-600" />
              </div>
              <h3 className="font-bold text-primary text-sm">Solicitudes recientes</h3>
            </div>
            <Link to="/solicitudes" className="text-xs text-accent hover:underline font-semibold flex items-center gap-1">
              Ver todas
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-50 border-b border-gray-100">
                  <th className="text-left px-5 py-3 text-[11px] font-bold text-slate-400 uppercase tracking-wider">ID</th>
                  <th className="text-left px-5 py-3 text-[11px] font-bold text-slate-400 uppercase tracking-wider">Cliente</th>
                  <th className="text-left px-5 py-3 text-[11px] font-bold text-slate-400 uppercase tracking-wider hidden md:table-cell">Vehículo</th>
                  <th className="text-left px-5 py-3 text-[11px] font-bold text-slate-400 uppercase tracking-wider hidden lg:table-cell">Servicio</th>
                  <th className="text-left px-5 py-3 text-[11px] font-bold text-slate-400 uppercase tracking-wider hidden lg:table-cell">Mecánico</th>
                  <th className="text-left px-5 py-3 text-[11px] font-bold text-slate-400 uppercase tracking-wider">Estado</th>
                </tr>
              </thead>
              <tbody>
                {recientes.length === 0 ? (
                    ...
                    <td colSpan={6} className="px-5 py-12 text-center">
                      <div className="flex flex-col items-center gap-2">
                        <div className="w-10 h-10 bg-slate-100 rounded-full flex items-center justify-center">
                          <Icon path="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" className="w-5 h-5 text-slate-400" />
                        </div>
                        <p className="text-slate-400 text-xs">No hay solicitudes registradas</p>
                      </div>
                    </td>
                  </tr>
                ) : recientes.map((s, idx) => (
                  <tr key={s.id} className={`border-b border-gray-50 hover:bg-slate-50 transition-colors ${idx === recientes.length - 1 ? 'border-b-0' : ''}`}>
                    <td className="px-5 py-3.5">
                      <span className="font-mono text-[11px] bg-slate-100 text-slate-500 px-2 py-0.5 rounded-md font-semibold">#{s.id}</span>
                    </td>
                    <td className="px-5 py-3.5 font-semibold text-slate-800 text-xs">{s.cliente}</td>
                    <td className="px-5 py-3.5 text-slate-500 hidden md:table-cell text-xs">{s.vehiculo}</td>
                    <td className="px-5 py-3.5 text-slate-500 hidden lg:table-cell text-xs max-w-[180px] truncate">{s.servicio}</td>
                    <td className="px-5 py-3.5 hidden lg:table-cell">
                      {s.mecanico
                        ? <span className="text-xs text-slate-600 font-medium">{s.mecanico.name}</span>
                        : <span className="text-xs text-amber-500 font-semibold bg-amber-50 px-2 py-0.5 rounded-full">Sin asignar</span>}
                    </td>
                    <td className="px-5 py-3.5">
                      <span className={`px-2.5 py-1 rounded-full text-[11px] font-bold ${estadoStyles[s.estado] || 'bg-slate-100 text-slate-600'}`}>
                        {s.estado}
                      </span>
                    </td>
                  </tr>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            <div className="flex items-center gap-2 mb-4">
              <div className="w-7 h-7 bg-violet-50 rounded-lg flex items-center justify-center">
                <Icon path="M11 3.055A9.001 9.001 0 1020.945 13H11V3.055z M20.488 9H15V3.512A9.025 9.025 0 0120.488 9z" className="w-3.5 h-3.5 text-violet-600" />
              </div>
              <h3 className="font-bold text-primary text-sm">Distribución</h3>
            </div>
            <div className="flex items-center gap-4">
              <DonutChart data={donutData} total={kpis.total} />
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            <div className="flex items-center gap-2 mb-4">
              <div className="w-7 h-7 bg-emerald-50 rounded-lg flex items-center justify-center">
                <Icon path="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" className="w-3.5 h-3.5 text-emerald-600" />
              </div>
              <h3 className="font-bold text-primary text-sm">Por estado</h3>
            </div>
